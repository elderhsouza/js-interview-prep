...
export function allAnagrams1(strings: string[]) {
...
export function allAnagrams2(strings: string[]): boolean {
  if(strings.length === 0) {
    return true;
  }
...
export function allAnagrams3(strings: string[]): boolean {
  if (strings.length === 0) {
    return false;
  }

  return new Set(
    strings.map(string => [...string].sort().join(''))
  ).size === 1;
}
...
export function allAnagrams4(strings: string[]): boolean {
  if (strings.length === 0) {
    return false;
  }
  ...
  return (
    String(charMapValues.next().value + ',')
